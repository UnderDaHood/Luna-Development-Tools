...
void ScanMap(std::string mapfile,bool forced=false) {
	...
	for (auto layname : *layers) {
		...
		if (config->BoolValue("ForcedZoneAction", mapfile)) {
			static std::vector<String> fzat {"enter", "cycle", "leave"}; // Forced Zone Action Type
			QCol->Doing("Forced zone actions", mapfile + "::" + layname);
			for (auto t : fzat) {
				auto acts = config->List("ForcedZoneAction", mapfile + "." + layname + "." + t);
				for (auto act : *acts) {
					ZoneAction += "\t_" + t + "[\"" + act + "\"] = \"" + act += "\"\t -- Forced Zone Action --\n";
					QCol->Doing("-> FCA." + t, act);
				}
			}
		}
		ZoneAction += "end\n\n";
